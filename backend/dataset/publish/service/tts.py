...
class TTSService:
    ...
    def load_tts_dataset(self, request):
        try:
            ...
            if record:
                result = self.get_enriched_tts_data(record, metadata)
                if result:
                    if result[0] == "INSERT":
                        ...
                        pt.update_task_details({"status": "SUCCESS", "serviceRequestNumber": metadata["serviceRequestNumber"],
                                                "durationInSeconds": record["durationInSeconds"], "datasetType": dataset_type_tts})
                    elif result[0] == "UPDATE":
                        pt.update_task_details({"status": "SUCCESS", "serviceRequestNumber": metadata["serviceRequestNumber"],
                                                "durationInSeconds": record["durationInSeconds"], "datasetType": dataset_type_tts, "isUpdate": True})
                        ...
                        error_list.append(
                            {"record": result[1], "code": "UPLOAD_FAILED", "datasetName": metadata["datasetName"],
                             "datasetType": dataset_type_tts, "serviceRequestNumber": metadata["serviceRequestNumber"],
                    ...
                    error_list.append(
                        {"record": record, "code": "INTERNAL_ERROR", "originalRecord": record,
                         "datasetType": dataset_type_tts, "datasetName": metadata["datasetName"],
                         "serviceRequestNumber": metadata["serviceRequestNumber"],
    ...
    '''
    def get_enriched_tts_data(self, data, metadata):
        try:
            hashes = [data["audioHash"], data["textHash"]]
            record = self.get_tts_dataset_internal({"tags": {"$all": hashes}})
            if record:
                if isinstance(record, list):
                    record = record[0]
                dup_data = service.enrich_duplicate_data(data, record, metadata, tts_immutable_keys, tts_updatable_keys, tts_non_tag_keys)
                if dup_data:
                    if metadata["userMode"] != user_mode_pseudo:
            ...
            for key in insert_data.keys():
                if key not in tts_immutable_keys and key not in tts_updatable_keys:
            ...
            insert_data["tags"] = service.get_tags(insert_data, tts_non_tag_keys)
            if metadata["userMode"] != user_mode_pseudo:
                epoch = eval(str(time.time()).replace('.', '')[0:13])
                s3_file_name = f'{metadata["datasetId"]}|{epoch}|{data["audioFilename"]}'
                object_store_path = utils.upload_file(data["fileLocation"], tts_prefix, s3_file_name)
    ...
    '''
    def get_tts_dataset_internal(self, query):
        try:
        ...
        try:
            ...
            if result:
                ...
                if path:
                    ...
                    pt.task_event_search(op, None, dataset_type_tts)
                else:
                    log.error(f'There was an error while pushing result to S3')
                    error = {"code": "OS_UPLOAD_FAILED", "datasetType": dataset_type_tts, "serviceRequestNumber": query["serviceRequestNumber"],
                                                   "message": "There was an error while pushing result to object store"}
                    op = {"serviceRequestNumber": query["serviceRequestNumber"], "userID": query["userId"],
                          "count": 0, "sample": [], "dataset": None, "datasetSample": None}
                    pt.task_event_search(op, error, dataset_type_tts)
            else:
    ...
    '''
    def delete_tts_dataset(self, delete_req):
        ...
        try:
            ...
            error = {"code": "DELETE_FAILED", "datasetType": dataset_type_tts,
